...
cal10ms = load_image_stack("FinalProject/Polarization/rotation2_86LP10ms", 10)  # 1 ms sky
cal20ms = load_image_stack("FinalProject/Polarization/rotation2_86LP20ms", 10)  # 2 ms sky
offset1ms = load_image_stack("FinalProject/Polarization/rotation2_offset1ms", 10)  # 1 ms, looking away from moon
print(f"cal10ms shape: {cal10ms.shape}")
print(f"cal20ms shape: {cal20ms.shape}")
print(f"offset1ms shape: {offset1ms.shape}")


# Show the first image from each dataset
show_images = True  # Set to False to skip showing images
if show_images:
    fig, axs = plt.subplots(1, 2, figsize=(10, 5))
    axs[0].imshow(cal10ms[0], cmap='gray')
    axs[0].set_title('cal10ms (First frame)')
    max_val_0 = np.max(cal10ms[0])
    axs[0].text(
        0.98, 0.02, f"Max: {max_val_0}", color='red', fontsize=20,
        ha='right', va='bottom', transform=axs[0].transAxes,
        bbox=dict(facecolor='black', alpha=0.5, edgecolor='none')
    )

    axs[1].imshow(cal20ms[0], cmap='gray')
    axs[1].set_title('cal20ms (First frame)')
    max_val_1 = np.max(cal20ms[0])
    axs[1].text(
        0.98, 0.02, f"Max: {max_val_1}", color='red', fontsize=20,
...
def sanuc(data1, data2):
    ...
    print(f"mu1 = {np.mean(mu1):.3f}, mu2 = {np.mean(mu2):.3f}")
    print(f"sigma1 = {np.mean(sigma1):.3f}, sigma2 = {np.mean(sigma2):.3f}")
...
G_sanuc, K_sanuc, B_sanuc = sanuc(cal10ms, cal20ms)
...
#K_nls = nls_nuc(cal10ms, cal20ms, 1, 2, offset)
...
exposure_sanuc_ms = 10.0  
exposure_nls_ms = 10.0    
